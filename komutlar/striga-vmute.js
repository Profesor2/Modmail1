...
exports.run = async (client, message, args) => {
  ...
  if (
    !["779619156697677844"].some(role => message.member.roles.cache.get(role)) &&
  ...
  const mutelog = message.guild.channels.cache.find(c => c.id === "791378260167622707"); //mute log
    ...
    setTimeout(async function() {
      ...
      mutelog.send(
        new MessageEmbed()
          ...
          .setDescription(
            `**Sesli Odalarda Susuturlması Kalktı !** \n**Yetkili:** ${
              message.author
            } (\`${message.author.id}\`)\n**Kullanıcı:** ${kullanici.user} (\`${
              kullanici.user.id
            }\`)\n**Süre:** \`${zaman1}\`\n**Sebep**: \`${sebep}\` \n**Tarih:** \`${moment(
